...
def renderDump (tests : List TestCase) (b : Builder) (es : EvalState)
  ...
  let evalCells : List String := (es.entries.toList).map fun
    ...
    | none   => "0"
  let mut out :=
    s!";; AUTOGENERATED — do not edit. {n} nodes, {tests.length} tests.\n\n"
  ...
  for t in tests do
    out := out ++ s!";; {t.name}\n"
    out := out ++ s!"!(assert (check-cert nil {t.termIdx} {t.typeIdx}))\n"
  return out
...
def runOne (name fileBase : String) (build : BuilderM (Nat × Nat)) : IO Unit := do
  ...
  let prog : EvalM Unit := do
    let _ ← evalM termIdx
    let _ ← evalM typeIdx
    -- Force-evaluate any leftover nodes so every entry is populated.
    let n := (← liftM (m := BuilderM) get).nodes.size
