...
impl<A> Ptr<A> {
    ...
    /// Re-type the phantom marker. The runtime data is just the `u32`, so
    /// changing `A` is a no-op at runtime and lets us move pointers between
    /// `ExportFile<'p>`s with different lifetime parameters during the flat
    /// (de)serialisation roundtrip in `zkvm_entry`.
    pub(crate) fn cast<B>(self) -> Ptr<B> { Ptr { raw: self.raw, ph: PhantomData } }
}

// Custom serde impls — `derive` doesn't work because of `PhantomData<A>`.
// We round-trip only the `u32`, dropping the phantom marker.
impl<A> Serialize for Ptr<A> {
    fn serialize<S: Serializer>(&self, s: S) -> Result<S::Ok, S::Error> {
        self.raw.serialize(s)
    }
}

impl<'de, A> Deserialize<'de> for Ptr<A> {
    fn deserialize<D: Deserializer<'de>>(d: D) -> Result<Self, D::Error> {
        Ok(Ptr { raw: u32::deserialize(d)?, ph: PhantomData })
    }
}
...
impl<'t, 'p: 't> TcCtx<'t, 'p> {
    ...
    /// Shim for the removed pretty/debug-printer modules. Returns its input
    /// unchanged so existing `format!("{:?}", ctx.debug_print(x))` call sites
    /// continue to compile — the output uses the derived `Debug` of the pointer.
    #[inline]
    pub(crate) fn debug_print<T>(&self, x: T) -> T { x }
...
impl Config {
    ...
    //
    // Kept as a native-host convenience; not used from inside the SP1 guest.
    pub fn to_export_file<'a>(self) -> Result<(ExportFile<'a>, Vec<String>), Box<dyn Error>> {
        if let Some(pathbuf) = self.export_file_path.as_ref() {
            ...
            panic!("Configuration file must specify an export file path or \"use_stdin\": true")
        }
    }
}
