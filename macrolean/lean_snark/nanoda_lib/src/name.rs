...
// `bound = ""` strips the default `'a: 'de` bounds — the lifetime is purely
// phantom (lives only in `NamePtr<'a>` / `StringPtr<'a>` which don't actually
// borrow), so the bounds aren't needed and would block deserialising as
// `Name<'static>` from a `'de` shorter than `'static`.
