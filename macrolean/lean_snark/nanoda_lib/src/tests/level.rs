...
// The two `debug_test*` tests previously asserted on pretty-printer output
// (e.g. "max(1, 1) + 1"); they were removed together with the pretty_printer
// module. The structural assertions they carried (`ctx.level_succs(..)`) are
// already covered above by `succ_test*` / `max_test*`.
