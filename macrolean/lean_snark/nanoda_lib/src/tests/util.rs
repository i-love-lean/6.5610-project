...
pub(crate) fn test_get_export_file<'p>(config_path: Option<&Path>) -> Result<(ExportFile<'p>, Vec<String>), Box<dyn Error>> {
    let config_file = match config_path {
        ...
        Some(_) => panic!("Config::try_from(path) was removed along with the pretty-printer — tests must construct Config inline"),
    };
...
// The `check_proj_from_prop` test was removed: it relied on
// `Config::try_from(&Path)` to read a JSON config, which was removed for the
// SP1 port.
