...
/-- Render a Lurk file that loads `slop/dependent.lurk` and runs every
test through `check_pair`, asserting success. -/
def renderLurkTests : String := Id.run do
  let mut out :=
    ";; AUTOGENERATED by `lake exe dependent --gen-lurk` — do not edit by hand.\n" ++
    ";; Loads the type checker and runs every test from Dependent.lean.\n\n" ++
    "!(load \"dependent.lurk\")\n\n"
  for entry in tests.toList do
    let name := entry.1
    let tpair := entry.2
    let safeName := name.replace "'" "_prime"
    out := out ++ s!";; ----- {name} -----\n"
    out := out ++ s!"!(def {safeName} {serialize tpair})\n"
...
def depMain (args : List String) : IO Unit := do
  if args == ["--gen-lurk"] then
    ...
    IO.println s!"Wrote {path} ({renderLurkTests.length} bytes)"
    return
